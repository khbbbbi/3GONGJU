<!-- 장바구니에 담기 버튼 누르면 장바구니에 똑같은 상품이 있는지 체크 -->
...
<%
	...
	try {      
        ...
        String sql = "select * from cart where userId = '"+state+"' AND breadID = '"+breadID+"'";
        ...
        if(found){
        	...
        	out.println("location.href='Cart.jsp?_userId="+state+"&_breadID="+breadID+"&_surrang="+surrang+"';");
        	out.println("} else { history.back();} </script>");
        }else{
        	//장바구니에 없으면 cart_insert.jsp로 값전달과 함께 이동!(없으니까 장바구니에 추가해주기 위함)
        	response.sendRedirect("cart_insert.jsp?_userId="+state+"&_breadID="+breadID+"&_surrang="+surrang+"");
        }
